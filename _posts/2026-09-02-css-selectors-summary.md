---
layout: post
title: "CSS 선택자 총정리 - 자식 선택자와 후손 선택자 헷갈림 정리"
date: 2026-09-02
categories: [CSS, 정리]
...
선택자는 특정한 HTML 요소를 선택할 때 사용하는 기능이다. 요소를 선택한 뒤 원하는 스타일(색상, 배경, 크기 등)을 적용할 수 있다. [실습 코드는 GitHub 저장소](https://github.com/yang7493/CSS-)에 파일별로 정리해두었고, 아래는 그 실습 코드를 기준으로 정리한 내용이다.
...
오늘 가장 헷갈렸던 개념이라 조금 더 깊게 정리했다. 실습 HTML 구조는 이랬다.
...
> **정리**: `>`는 "내 바로 아래 자식만", 공백은 "내 안에 있으면 몇 단계든 전부"라고 구분하면 헷갈리지 않는다.

## 4. 동위(형제) 선택자

부모-자식 관계가 아니라, **같은 부모를 가진 형제 요소**를 선택하는 방법이다.
...
형제 관계 안에서 **순서(위치)**로 요소를 선택하는 방법이다.
...
사용자의 행동이나 입력 양식의 상태에 따라 달라지는 선택자다.
...
`:hover`, `:focus`처럼 콜론 1개(`:`)로 쓰는 건 **가상 클래스**(요소의 상태), `::first-letter`처럼 콜론 2개(`::`)로 쓰는 건 **가상 요소**(요소 안의 특정 부분)라는 이름 차이도 함께 정리해두면 좋을 것 같다.
